...
def miller_rabin(n , pass_no = None):
	...
	for i in range(0,pass_no):
		while prob_prime == False:
			...
			relation ship with the number being tested such 
...
def keys(identifier, bit_size=None):
	"""
	...
	# (remove comment when resolved) The below line may be a slightly bloated way of doing things.
...
def RSA_string_encrypt(message_block, key_ring):
	pass
	"""
...
#test = keys(1028)
#text = "hello world"
#secret = RSA_string_encrypt(text, test)
...
def RSA_string_decrypt(message_block, key_ring):
	"""
...
print ''
#print RSA_string_decrypt(secret, test)
