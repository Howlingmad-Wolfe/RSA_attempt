...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------		
	...
	def v1_5_like_padding(self):
		...
		for b in self.message:
			...
			pad_len = self.byte_block_len - ( int( b ).bit_length() / 8 )
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	# self.byte_block_len == self.key_ring[ 'modulus' ].bit_length() / 8 == ~ 2048 bits / 8 == ~ 255 -- 256 bytes
	# 1 byte = 8 bits
	# need to leave 1 byte for pad length indicator
	...
	def pad ( self ):
		...
		for data in self.message:
			data_length = int( data ).bit_length() / 8

			print data_length, " data length bytes"

			pad_len = (self.byte_block_len - data_length) / 8 # ~255 - 86 == ~169
		
			print pad_len, " pad len"

			padding = int(os.urandom( pad_len - 1 ).encode( 'hex' ), 16 )

			print padding.bit_length() / 8, " padding byte length"

			pad_len = len( str ( padding ) )
			
			# Making sure that 'pad_len' is three integers long for accurate reading and removal later
			pad_len = len(str( padding ) )
			...
			padded_blocks.append( str( padding ) + data + str( pad_len ) )
...
#print pow( hello.key_ring["modulus"], 1, hello.key_ring["lock"] )
print ""
